...
map_version_number_to_android_codename <- function(version_number) {
    if(str_detect(version_number, "^[^0-9]*1\\.6(\\.[0-9])?")) {
        ...
        "eclair"
    } else if(str_detect(version_number, "^[^0-9]*2\\.2(\\.[0-3])?")) {
        "froyo"
    } else if(str_detect(version_number, "^[^0-9]*2\\.3(\\.[0-7])?")) {
        "gingerbread"
    } else if(str_detect(version_number, "^[^0-9]*3\\.[0-2](\\.[0-6])?")) {
        "honeycomb"
    } else if(str_detect(version_number, "^[^0-9]*4\\.0(\\.[0-5])?")) {
        "ice cream sandwich"
    } else if(str_detect(version_number, "^[^0-9]*4\\.[1-3](\\.[0-1])?")) {
        "jellybean"
    } else if(str_detect(version_number, "^[^0-9]*4\\.4(\\.[0-4])?")) {
        "kitkat"
    } else if(str_detect(version_number, "^[^0-9]*5\\.[0-1](\\.[0-1])?")) {
        "lollipop"
    } else if(str_detect(version_number, "^[^0-9]*6\\.0(\\.[0-1])?")) {
        "marshmallow"
    } else if(str_detect(version_number, "^[^0-9]*7\\.[0-1](\\.[0-2])?")) {
        "nougat"
    } else if(str_detect(version_number, "^[^0-9]*8\\.[0-1](\\.[0-9])?")) {
        "oreo"
    } else {
        "unknown"
    }
